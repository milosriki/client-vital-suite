...
async function auditDialogflowHistory() {
  console.log("🔍 Starting Dialogflow History Audit...");
  ...
  // distinct check on metadata->handoff or checking text for "checking with finance"
  ...
  console.log(
    `🤖 Potential Atlas Handoffs detected (by text): ${handoffs?.length || 0}`,
  );

  // 3. Check for Rule Violations (Business Decisions)
  ...
  if (violations && violations.length > 0) {
    ...
    console.log(
      "\n✅ No obvious business keyword violations found in recent history.",
    );
  }
