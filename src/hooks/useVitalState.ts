...
export function useVitalState(): VitalStateReturn {
  ...
  useEffect(() => {
    ...
    // Subscribe to sync_errors changes
    const errorsChannel = supabase
      .channel('vital-sync-errors')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'sync_errors' },
        () => {
  ...
  const systemHealth = useMemo<VitalSystemHealth>(() => {
    const isHealthy = (healthData?.activeErrors || 0) === 0;
    
    return {
      isHealthy,
      lastSyncTime: healthData?.lastSyncTime || null,
      activeErrors: healthData?.activeErrors || 0,
      connectionStatus: 'connected', // Supabase handles reconnection automatically
    };
  }, [healthData]);
  ...
  return {
    systemHealth,
    isLoading,
    isConnected: true, // Supabase maintains connection
    invalidateAll,
    invalidateDashboard,
    invalidatePipeline,
    invalidateClients,
