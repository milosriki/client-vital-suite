...
export default function TeamLeaderboard() {
  const { data: teamData, isLoading } = useDedupedQuery({
    ...
    queryFn: async () => {
      ...
      if (callError || leadError || dealError || staffError)
        throw new Error("Data fetch error");

      const staffMap = Object.fromEntries(
        staff?.map((s) => [s.hubspot_owner_id || s.id, s]) || [],
      );
      ...
      setLeads?.forEach((l) => {
        ...
        if (
          person &&
          (l.status === "appointment_set" || l.status === "appointment_held")
        ) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            Team Performance War Room
          </h1>
          <p className="text-muted-foreground">
            Monthly Rankings: Setters vs. Coaches Performance
          </p>
        </div>
            ...
            <CardContent className="space-y-6">
              {(Array.isArray(teamData) ? teamData : [])
                ...
                .map((member: any, index) => (
                  <div
                    key={member.name}
                    className="flex items-center gap-4 p-4 rounded-xl bg-muted/30 border border-border/50 relative overflow-hidden"
                  >
                    ...
                    {index === 0 && (
                      <Zap className="absolute -right-2 -top-2 h-12 w-12 text-primary/10 rotate-12" />
                    )}
                  </div>
                ))}
            </CardContent>
        ...
        <div className="xl:col-span-2">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm h-full">
            <CardHeader>
              ...
              <CardDescription>
                Comprehensive activity and outcome tracking
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  <TableHeader className="bg-muted/50">
                    <TableRow>
                      ...
                      <TableHead className="text-right font-bold text-green-400">
                        Revenue
                      </TableHead>
                    </TableRow>
                  </TableHeader>
