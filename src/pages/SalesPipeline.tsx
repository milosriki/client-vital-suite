...
export default function SalesPipeline() {
  ...
  const { data: callRecords } = useDedupedQuery({
    ...
    queryFn: async () => {
      const dateFilter = getDateFilter();
      let query = supabase.from("call_records").select("id, caller_number, call_status, call_outcome, duration_seconds, lead_quality, transcription, recording_url, appointment_set, created_at, owner_name, call_type, call_direction");
      if (dateFilter) {
        query = query.gte("created_at", dateFilter);
      }
      const { data, error } = await query.order("created_at", {
