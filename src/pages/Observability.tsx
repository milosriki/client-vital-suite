...
import { RefreshCw, Activity, DollarSign, Clock, AlertTriangle, CheckCircle, XCircle, ExternalLink } from "lucide-react";
...
interface AggregatedStats {
  ...
  avgLatency: number;
  totalCost: number;
  byFunction: Record<string, { count: number; errors: number; avgLatency: number; cost: number }>;
...
export default function Observability() {
  ...
  const calculateStats = (data: AIMetric[]) => {
    const byFunction: Record<string, { count: number; errors: number; avgLatency: number; cost: number; totalLatency: number }> = {};
    ...
    data.forEach((m) => {
      ...
      if (!byFunction[m.function_name]) {
        byFunction[m.function_name] = { count: 0, errors: 0, avgLatency: 0, cost: 0, totalLatency: 0 };
      }
      ...
      if (m.provider) {
        ...
        byProvider[m.provider].tokens += (m.tokens_in || 0) + (m.tokens_out || 0);
        if (m.cost_usd_est) byProvider[m.provider].cost += Number(m.cost_usd_est);
      }
    ...
    Object.keys(byFunction).forEach((fn) => {
      byFunction[fn].avgLatency = byFunction[fn].count > 0 ? byFunction[fn].totalLatency / byFunction[fn].count : 0;
    });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "timeout":
        return <Badge className="bg-yellow-500"><Clock className="w-3 h-3 mr-1" />Timeout</Badge>;
      default:
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Monitor AI function executions, costs, and performance</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {stats ? `${Math.round((stats.totalCalls * stats.successRate) / 100)} successful` : ""}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(stats?.avgLatency || 0).toFixed(0)}ms</div>
          ...
          <CardContent>
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {stats?.byFunction && Object.entries(stats.byFunction)
                ...
                .slice(0, 15)
                .map(([fn, data]) => (
                  <div key={fn} className="flex items-center justify-between p-2 bg-muted/50 rounded">
                    <div className="flex-1">
                      <code className="text-sm font-mono">{fn}</code>
                      <div className="flex gap-2 text-xs text-muted-foreground">
                        <span>{data.count} calls</span>
                        <span>•</span>
                        <span className={data.errors > 0 ? "text-red-500" : ""}>{data.errors} errors</span>
                        <span>•</span>
                        <span>{data.avgLatency.toFixed(0)}ms avg</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-medium">${data.cost.toFixed(4)}</span>
                    </div>
                  </div>
                ))}
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {stats?.byProvider && Object.entries(stats.byProvider)
              ...
              {(!stats?.byProvider || Object.keys(stats.byProvider).length === 0) && (
                <p className="text-muted-foreground text-center py-4">No provider data yet</p>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {metrics.slice(0, 50).map((m) => (
              <div key={m.id} className="flex items-center gap-4 p-3 bg-muted/30 rounded-lg hover:bg-muted/50 transition">
                {getStatusBadge(m.status)}
                <div className="flex-1 min-w-0">
                  <code className="text-sm font-mono truncate block">{m.function_name}</code>
                  <div className="flex gap-2 text-xs text-muted-foreground">
                    {m.provider && <span className="capitalize">{m.provider}</span>}
                    {m.model && <span>• {m.model}</span>}
                    {m.latency_ms && <span>• {m.latency_ms}ms</span>}
                    {m.tokens_in && m.tokens_out && (
                      <span>• {m.tokens_in}/{m.tokens_out} tokens</span>
                    )}
                  </div>
                </div>
                <div className="text-right text-xs text-muted-foreground">
                  {m.cost_usd_est && <div>${Number(m.cost_usd_est).toFixed(6)}</div>}
                  <div>{new Date(m.created_at).toLocaleTimeString()}</div>
                </div>
                {m.error_message && (
                  <AlertTriangle className="w-4 h-4 text-red-500" title={m.error_message} />
                )}
              </div>
            ))}
            {metrics.length === 0 && !loading && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">AI functions will appear here once they run</p>
              </div>
            )}
          </div>
        </CardContent>
