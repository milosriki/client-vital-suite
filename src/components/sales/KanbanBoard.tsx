...
export function KanbanBoard({ leads, onMoveCard }: KanbanBoardProps) {
  ...
  const totalPipelineValue = useMemo(() => {
    return leads.reduce((sum, l) => sum + (Number(l.deal_value) || Number(l.amount) || 0), 0);
  }, [leads]);
      ...
      <CardContent className="pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {STAGES.map((stage) => {
            ...
            const totalValue = stageLeads.reduce((sum, l) => sum + (Number(l.deal_value) || Number(l.amount) || 0), 0);
