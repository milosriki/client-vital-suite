...
export const useWarRoomData = () => {
  ...
  const discountedDeals =
    deals?.filter((d) => {
      const dealValue = Number(d.deal_value) || Number(d.amount) || 0;
  ...
  const getHistoricalRevenue = () => {
    ...
    for (let i = 2; i >= 0; i--) {
      ...
      const monthRevenue = closedDeals
        ...
        .reduce((sum, d) => sum + (Number(d.deal_value) || Number(d.amount) || 0), 0);
  ...
  const generateForecastData = () => {
    ...
    const commitValue = contractDeals.reduce(
      (sum, d) => sum + (Number(d.deal_value) || Number(d.amount) || 0) * 0.9,
      0,
    );
    const likelyValue = proposalDeals.reduce(
      (sum, d) => sum + (Number(d.deal_value) || Number(d.amount) || 0) * 0.6,
      0,
    );
    const bestCaseValue = activeDeals.reduce(
      (sum, d) => sum + (Number(d.deal_value) || Number(d.amount) || 0) * 0.3,
      0,
    );
