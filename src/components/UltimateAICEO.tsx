...
export function UltimateAICEO() {
  ...
  const { data: integrationStatus } = useQuery({
    ...
    queryFn: async () => {
      ...
      const status: Record<
        string,
        { connected: boolean; lastSync: string | null; errors: number }
      > = {};
  ...
  const rejectAction = useMutation({
    ...
    onSuccess: () => {
      toast.info("Action rejected", {
        description: "AI will learn from this decision",
      });
      queryClient.invalidateQueries({ queryKey: ["pending-actions"] });
      setSelectedAction(null);
      setRejectionReason("");
  ...
  const stats = {
    ...
    autoApprovable:
      pendingActions?.filter(
        (a) => a.risk_level === "low" && a.confidence > 0.85,
      ).length || 0,
  };
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2 flex-wrap">
                                ...
                                <Badge
                                  className={getRiskColor(action.risk_level)}
                                >
                                  {action.risk_level}
                                </Badge>
                                <Badge
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              {(!insights || insights.length === 0) && (
                <div className="text-center py-12 text-white/40">
                  ...
                  <p>
                    No proactive insights yet. Click "Run Monitor" to generate.
                  </p>
                </div>
              )}
            </div>
                  ...
                  <div className="space-y-4">
                    {calibrationData?.map((item) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <Badge
                            variant={
                              item.was_ai_correct ? "default" : "destructive"
                            }
                          >
                    ...
                    {(!calibrationData || calibrationData.length === 0) && (
                      <p className="text-center text-white/40 py-8">
                        No calibration data yet. Approve or reject actions to
                        teach the AI.
                      </p>
                    )}
                  </div>
