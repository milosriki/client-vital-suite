...
export default function PTDControlChat() {
  ...
  const handleToggleGlobalMode = () => {
    ...
    toast.success(
      newMode ? "Switched to Global Brain" : "Switched to Personal Mode",
    );
      ...
      try {
        ...
        if (error) {
          ...
          setUploadedFiles((prev) => [
            ...prev,
            { name: file.name, content: content.slice(0, 500) },
          ]);
    ...
    try {
      ...
      if (!response.ok || data?.error) {
        ...
        if (voiceEnabled && voiceOutputSupported) {
          const textToSpeak = String(aiResponse || "")
            .replace(/[#*`_]/g, "")
            .substring(0, 500);
          speak(textToSpeak);
        }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={
                isGlobalMode
                  ? "Global Brain (shared with everyone)"
          ...
          <div className="mt-2 flex items-center gap-2 text-xs text-white/30">
            {isGlobalMode ? (
              <>
                ...
                {userLabel && (
                  <span className="text-white/50">({userLabel})</span>
                )}
              </>
            ) : (
        ...
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center rounded-2xl z-10">
          <div className="bg-slate-800 border border-cyan-500/30 rounded-xl p-6 max-w-xs w-full mx-4">
            ...
            <p className="text-white/60 text-sm mb-4">
              Enter your name so others know who contributed to the shared
              memory.
            </p>
            <input
              type="text"
