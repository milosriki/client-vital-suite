...
function calculateHealthScore(contact: HubSpotContact): HealthScoreResult {
  ...
  const sessions7d = parseFloat(props[HUBSPOT_PROPERTIES.sessions_7d] || '0') || 0;
  const sessions30d = parseFloat(props[HUBSPOT_PROPERTIES.sessions_30d] || '0') || 0;
  ...
  const purchased = parseFloat(props[HUBSPOT_PROPERTIES.sessions_purchased] || '0') || 0;
  const remaining = parseFloat(props[HUBSPOT_PROPERTIES.outstanding_sessions] || '0') || 0;
  ...
  const isBooked = nextSessionBooked === 'Y' || 
                   nextSessionBooked === 'Yes' || 
                   nextSessionBooked === 'true' || 
                   nextSessionBooked === '1';
  if (isBooked) {
    const futureBooked = parseFloat(props[HUBSPOT_PROPERTIES.future_booked_sessions] || '0') || 0;
...
async function batchUpdateHealthScores(
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error(`HubSpot batch update error: ${response.status} - ${errorText}`);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[calculate-health-scores] Starting run: cursor=${cursor || 'none'}, dry_run=${dry_run}`);
    ...
    while (processedCount < MAX_RECORDS_PER_RUN) {
      ...
      console.log(`[calculate-health-scores] Processing batch of ${contacts.length} contacts`);

      // Calculate health scores for batch
      ...
      for (const contact of contacts) {
        try {
          ...
          switch (result.healthZone) {
            case 'PURPLE': summary.zones.purple++; break;
            case 'GREEN': summary.zones.green++; break;
            case 'YELLOW': summary.zones.yellow++; break;
            case 'RED': summary.zones.red++; break;
          }
        } catch (err: unknown) {
          const errorMessage = err instanceof Error ? err.message : String(err);
          console.error(`[calculate-health-scores] Error calculating for contact ${contact.id}: ${errorMessage}`);
          summary.errors.push(`Contact ${contact.id}: ${errorMessage}`);
        }
      }
      ...
      if (!dry_run && healthResults.length > 0) {
        const { success, failed } = await batchUpdateHealthScores(HUBSPOT_API_KEY, healthResults);
        summary.updated += success;
        summary.failed += failed;
        console.log(`[calculate-health-scores] Batch update: ${success} success, ${failed} failed`);
      } else if (dry_run) {
        console.log(`[calculate-health-scores] Dry run - would update ${healthResults.length} contacts`);
        // Log sample results in dry run
        healthResults.slice(0, 5).forEach(r => {
          console.log(`  ${r.email}: score=${r.healthScore}, zone=${r.healthZone}`);
        });
      }
    ...
    console.log(`[calculate-health-scores] Complete: ${JSON.stringify(summary)}`);
