...
): Promise<{ response: string; thoughtSignature?: string }> {
...
- You MUST output your internal reasoning in a <thinking>...</thinking> block before your final answer.
- Trace your logic, analyze the data inconsistencies, and justify your conclusions.
- This is critical for the "AI Learning" system to improve.
...
`;
  ...
  let currentThoughtSignature = initialThoughtSignature;
  ...
  // Capture signature from first response
  currentThoughtSignature = currentResponse.thoughtSignature;
  ...
  while (
    ...
    currentResponse = await unifiedAI.chat(messages, {
      ...
      tools: agentTools,
      thinkingLevel: thinkingLevel || "high",
      thoughtSignature: currentThoughtSignature, // Pass forward the signature
    });

    // Update signature if new one provided
    if (currentResponse.thoughtSignature) {
      currentThoughtSignature = currentResponse.thoughtSignature;
    }
  }
  ...
  const finalThought =
    currentResponse.thought || "(No deep thinking trace detected)";

  // Log to Decision Engine (persist intelligence)
  try {
    await supabase.from("agent_decisions").insert({
      decision_type: isWhatsApp ? "WHATSAPP_RESPONSE" : "CEO_ANALYSIS",
      agent_id: "ptd-agent-gemini",
      input_context: {
      ...
      decision_output: {
        ...
        thought_process: finalThought,
      },
      outcome: "successful",
      status: "completed",
      confidence_score: finalThought.length > 50 ? 0.95 : 0.7, // Heuristic based on depth of thought
    });
    // Also update patterns if thought is rich
    if (finalThought.length > 100) {
      await supabase.rpc("increment_pattern_usage", {
        pattern_name: "deep_reasoning",
      });
    }
  } catch (logErr) {
    console.error("Decision logging error:", logErr);
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const thinkingLevel = body.thinkingLevel || context.thinkingLevel || "high";
    const thoughtSignature = body.thoughtSignature || context.thoughtSignature;
    ...
    const agentResult = await Promise.race([
      ...
      new Promise<{ response: string; thoughtSignature?: string }>(
        (_, reject) =>
          setTimeout(
            () => reject(new Error("Request timeout after 55s")),
            55000,
          ),
      ),
    ]);
    ...
    const newThoughtSignature = agentResult.thoughtSignature;

    const duration = Date.now() - startTime;
    console.log(`✅ Response generated in ${duration}ms`);

    // Sync to HubSpot (Background Task - Latency Optimization)
    if (context.source === "whatsapp" || context.platform === "whatsapp") {
      // FIRE AND FORGET: Do not await this promise.
      // This ensures 0ms latency impact on the user response.
    ...
    if (isWhatsAppReq) {
      ...
      if (!safety.isSafe) {
        console.error(`⚠️ BLOCKED UNSAFE RESPONSE:`, safety.issues);
        // FORCE FALLBACK (Business Firewall Enforcement)
        safeResponse =
          "hey || i need to double check that with the team to be sure. let me ask atlas and get back to you.";
      }
    }
    ...
    return apiSuccess({
      ...
      thread_id: threadId,
      duration_ms: duration,
      thoughtSignature: newThoughtSignature, // Return for client persistence
    });
  } catch (error: unknown) {
    const duration = Date.now() - startTime;
