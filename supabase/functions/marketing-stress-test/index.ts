...
serve(async (req) => {
  ...
  try {
    ...
    allContacts?.forEach((contact: any) => {
      const source = contact.utm_source || contact.first_touch_source || 'unknown';
      if (!sourceStats[source]) {
        sourceStats[source] = { leads: new Set(), deals: 0, revenue: 0 };
      }
      sourceStats[source].leads.add(contact.email);

      const deals = allDeals?.filter(d => d.hubspot_contact_id === contact.hubspot_contact_id) || [];
      if (deals.length > 0) {
    ...
    const q1Data = Object.entries(sourceStats)
      .map(([source, stats]) => ({
        ...
        ltv_per_lead: stats.leads.size > 0 ? stats.revenue / stats.leads.size : 0
      }))
    ...
    results.push({
      ...
      recommendations: q1TopSource?.ltv_per_lead > 10000 
      ...
      status: (q1TopSource?.ltv_per_lead || 0) > 10000 ? 'excellent' : (q1TopSource?.ltv_per_lead || 0) > 5000 ? 'good' : 'warning'
    });
    ...
    const qualifiedLeadsByCampaign: Record<string, { total: number; qualified: number }> = {};
    q2Data?.forEach((contact: any) => {
      ...
      if (['salesqualifiedlead', 'opportunity', 'customer'].includes(contact.lifecycle_stage)) {
        qualifiedLeadsByCampaign[campaign].qualified++;
      }
    });

    const q2Results = campaignSpend?.map(cp => {
      const leads = qualifiedLeadsByCampaign[cp.campaign_name] || { total: 0, qualified: 0 };
      const costPerQualified = leads.qualified > 0 ? cp.spend / leads.qualified : null;
      return {
        ...
        qualification_rate: leads.total > 0 ? (leads.qualified / leads.total) * 100 : 0
      };
    }).filter(r => r.qualified_leads > 0).sort((a, b) => (a.cost_per_qualified || Infinity) - (b.cost_per_qualified || Infinity));
    ...
    results.push({
      ...
      recommendations: (q2Best?.cost_per_qualified || 0) < 2000
        ? [`Scale ${q2Best?.campaign} - Low cost per qualified lead`]
        : [`Optimize campaigns - Cost per qualified lead is high`],
      status: (q2Best?.cost_per_qualified || 0) < 2000 ? 'excellent' : (q2Best?.cost_per_qualified || 0) < 4000 ? 'good' : 'warning'
    });
    ...
    const creativePerformance: Record<string, { leads: Set<string>; deals: Set<string>; revenue: number }> = {};
    
    q3Data?.forEach((event: any) => {
      const adId = event.fb_ad_id;
      if (!creativePerformance[adId]) {
        creativePerformance[adId] = { leads: new Set(), deals: new Set(), revenue: 0 };
      }
      creativePerformance[adId].leads.add(event.email);
    });

    contacts?.forEach((contact: any) => {
      const matchingDeals = deals?.filter(d => d.hubspot_contact_id === contact.hubspot_contact_id) || [];
      matchingDeals.forEach(deal => {
        Object.keys(creativePerformance).forEach(adId => {
          if (creativePerformance[adId].leads.has(contact.email)) {
            ...
            creativePerformance[adId].revenue += parseFloat(deal.deal_value || 0);
          }
        });
      });
    });
    ...
    const q3Results = Object.entries(creativePerformance)
      .map(([adId, data]) => ({
        ...
        conversion_rate: data.leads.size > 0 ? (data.deals.size / data.leads.size) * 100 : 0
      }))
      .filter(r => r.leads >= 5)
    ...
    results.push({
      ...
      recommendations: (q3Best?.conversion_rate || 0) > 20
        ? [`Scale creative ${q3Best?.ad_id} - High conversion rate`]
        : [`Test new creatives - Current conversion rates are low`],
      status: (q3Best?.conversion_rate || 0) > 20 ? 'excellent' : (q3Best?.conversion_rate || 0) > 10 ? 'good' : 'warning'
    });
    ...
    q4HubSpot?.forEach((c: any) => {
      if (!campaignCounts[c.utm_campaign]) {
        campaignCounts[c.utm_campaign] = { facebook: new Set(), hubspot: new Set(), anytrack: new Set() };
      }
    ...
    q4AnyTrack?.forEach((a: any) => {
      if (!campaignCounts[a.campaign]) {
        campaignCounts[a.campaign] = { facebook: new Set(), hubspot: new Set(), anytrack: new Set() };
      }
      campaignCounts[a.campaign].anytrack.add(a.email);
    });

    const q4Results = Object.entries(campaignCounts).map(([campaign, data]) => {
    ...
    results.push({
      question: "Q4: What's the ATTRIBUTION DISCREPANCY between Facebook, HubSpot, and AnyTrack?",
      answer: {
      ...
      recommendations: (q4Worst?.discrepancy_pct || 0) > 30
      ...
      status: (q4Worst?.discrepancy_pct || 0) > 30 ? 'critical' : (q4Worst?.discrepancy_pct || 0) > 15 ? 'warning' : 'good'
    });
    ...
    q5Contacts?.forEach((c: any) => {
      if (campaignMetrics[c.utm_campaign]) {
        if (['salesqualifiedlead', 'opportunity', 'customer'].includes(c.lifecycle_stage)) {
          campaignMetrics[c.utm_campaign].qualified++;
        }
        const deal = q5Deals?.find(d => d.hubspot_contact_id === c.hubspot_contact_id);
        if (deal) {
          campaignMetrics[c.utm_campaign].closed++;
          campaignMetrics[c.utm_campaign].revenue += parseFloat(deal.deal_value || 0);
        }
      }
    });

    const q5Results = Object.entries(campaignMetrics).map(([campaign, metrics]) => {
      const cpl = metrics.leads > 0 ? metrics.spend / metrics.leads : 0;
      const qualificationRate = metrics.leads > 0 ? (metrics.qualified / metrics.leads) * 100 : 0;
      const closeRate = metrics.leads > 0 ? (metrics.closed / metrics.leads) * 100 : 0;
      ...
      return {
        ...
        roas,
        status: cpl < 50 && closeRate < 5 ? 'BLEEDING' : cpl < 50 && closeRate < 10 ? 'WARNING' : roas < 2 ? 'LOW_ROI' : 'HEALTHY'
      };
    }).sort((a, b) => {
    ...
    results.push({
      question: "Q5: Which campaigns have HIGH FRONTEND METRICS but LOW BACKEND CONVERSION?",
      answer: {
      ...
      recommendations: q5Bleeding.length > 0
    ...
    // ========================================
    const q6Results = q5Results.map(r => ({
      campaign: r.campaign,
      roas: r.roas,
      status: r.roas >= 5 ? 'SCALE' : r.roas >= 3 ? 'GOOD' : r.roas >= 2 ? 'BREAK_EVEN' : 'LOSING_MONEY'
    ...
    results.push({
      ...
      recommendations: (q6Top?.roas || 0) >= 5
        ? [`SCALE ${q6Top?.campaign} - Excellent ROAS`]
        : [`Optimize campaigns - Target ROAS should be 5x+`],
      status: (q6Top?.roas || 0) >= 5 ? 'excellent' : (q6Top?.roas || 0) >= 3 ? 'good' : 'warning'
    });
