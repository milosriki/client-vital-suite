...
serve(async (req) => {
  ...
  try {
    ...
    if (!fbAccessToken) {
      throw new Error(
        "FB_ACCESS_TOKEN not configured in Edge Function Secrets",
      );
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        currency,
        date_preset,
        total_spend: campaignBreakdown.reduce(
          (sum: number, c: any) => sum + parseFloat(c.spend),
          0,
        ),
        breakdown: campaignBreakdown.map((c: any) => ({
