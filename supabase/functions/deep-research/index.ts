...
async function queryClientHealth(supabase: any, filters?: any): Promise<any> {
  ...
  return {
    ...
    zones,
    atRisk:
      data?.filter((c: any) => (c.health_zone || "").toLowerCase() === "red") ||
      [],
    topPerformers:
      data?.filter(
        (c: any) => (c.health_zone || "").toLowerCase() === "purple",
      ) || [],
  };
}
...
async function queryCoachPerformance(supabase: any): Promise<any> {
  ...
  return {
    ...
    avgPerformance: data?.length
      ? (
          data.reduce(
            (sum: number, c: any) => sum + (c.performance_score || 0),
            0,
          ) / data.length
        ).toFixed(1)
      : 0,
...
async function queryStripeData(supabase: any): Promise<any> {
  ...
  return {
    ...
    activeSubscriptions:
      subscriptions.data?.filter((s: any) => s.status === "active") || [],
...
async function queryCallRecords(supabase: any): Promise<any> {
  ...
  return {
    ...
    avgDuration: data?.length
      ? (totalDuration / data.length / 60).toFixed(1)
      : 0,
...
async function queryDailySummary(supabase: any): Promise<any> {
  ...
  return {
    ...
    trend:
      data?.length > 1
        ? (data[0]?.avg_health_score || 0) -
          (data[data.length - 1]?.avg_health_score || 0)
        : 0,
  };
}
...
async function callGemini(
  ...
  if (!GEMINI_API_KEY)
    throw new Error(
      "GEMINI_API_KEY, GOOGLE_API_KEY, or GOOGLE_GEMINI_API_KEY not configured",
    );
...
`;
  ...
  const planResponse = await callGemini(
    planPrompt,
    "You are a research planning agent. Return only valid JSON.",
  );
  ...
  try {
    ...
    const plan = match
      ? JSON.parse(match[0])
      : ["Analyze overall health", "Review financials", "Generate insights"];
...
async function executeResearchStep(
  ...
  if (
    stepLower.includes("health") ||
    stepLower.includes("client") ||
    stepLower.includes("churn") ||
    stepLower.includes("risk")
  ) {
    data.clientHealth = await queryClientHealth(supabase);
  }
  if (
    stepLower.includes("deal") ||
    stepLower.includes("revenue") ||
    stepLower.includes("pipeline") ||
    stepLower.includes("financial")
  ) {
    data.deals = await queryDeals(supabase);
  }
  if (
    stepLower.includes("coach") ||
    stepLower.includes("performance") ||
    stepLower.includes("trainer")
  ) {
    data.coaches = await queryCoachPerformance(supabase);
  }
  if (
    stepLower.includes("stripe") ||
    stepLower.includes("payment") ||
    stepLower.includes("fraud") ||
    stepLower.includes("subscription")
  ) {
    data.stripe = await queryStripeData(supabase);
  }
  if (
    stepLower.includes("intervention") ||
    stepLower.includes("action") ||
    stepLower.includes("recommendation")
  ) {
  ...
  if (
    stepLower.includes("trend") ||
    stepLower.includes("daily") ||
...
`;
  ...
  const insightResponse = await callGemini(
    insightPrompt,
...
// ============= MAIN GRAPH RUNNER =============
async function runResearchGraph(
  query: string,
  supabase: any,
): Promise<ResearchState> {
  ...
  try {
    ...
    await supabase.from("agent_memory").insert({
      ...
      knowledge_extracted: {
        ...
        avg_confidence:
          state.findings.reduce((sum, f) => sum + f.confidence, 0) /
    ...
    await logError(supabase, "deep-research", error, ErrorCode.INTERNAL_ERROR, {
      operation: "runResearchGraph",
      query,
    });
    return {
      ...state,
...
serve(async (req) => {
  ...
  try {
    ...
    return handleError(error, "deep-research", {
